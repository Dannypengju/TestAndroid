package com.example.testandroid.xia.datetime.wheelview;
...
public class WheelView extends View {
    ...
    private static final float SPECULAR_LIGHT = 300;
    /** Shininess constant */
    ...
    //currentItem 就是item的值，这个控件中涉及的都是数字所以 也就不要String咯
...
//		drawShadows(canvas);
    ...
    private void prepareMatrix(final Matrix outMatrix, int distanceY, int r, View child){
        //clip the distance
        double degree;
        double radians;
        float translateZ;
        final int d = Math.min(r, Math.abs(distanceY));
        translateZ = (float) Math.sqrt((r * r) - (d * d));
        if(distanceY > 0){
            radians = Math.acos((float) d / r);
            degree = 90 - (180 / Math.PI) * radians;
        }else{
            radians = Math.acos((float) d / r);
            degree = -(90 - (180 / Math.PI) * radians);
        }
        ...
        if(distanceY > -20 && distanceY < 20){
            mCamera.translate((r - translateZ) / 2, 0, -150);
        }else{
            mCamera.translate((r - translateZ) / 2, 0, r - translateZ);
        }
...
//        mPaint.setAlpha(getAlpha((float)degree));
    }
	...
	private void doScroll(int delta) {
		scrollingOffset += delta;    //delta 是Y轴移动的距离, scrollingOffset 初始值为0;

		int count = scrollingOffset / getItemHeight(); //count 移动的行数
		int pos = currentItem - count; //当前移动的位置
